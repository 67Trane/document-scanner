...
def move_pdf_to_customer_folder(pdf_path: str, customer) -> str:
    ...
    customer_dir = os.path.join(base_dir, f"{customer.customer_number}_{customer.last_name}")
...
def move_pdf_to_unassigned_folder(pdf_path: str) -> str:
    ...
    if not base_dir:
        raise ValueError("UNASSIGNED_DOCUMENT_ROOT is not configured in Django settings.")
